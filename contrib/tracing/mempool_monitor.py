...
def main(pid):
    print(f"Hooking into freycoind with pid {pid}")
    freycoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    freycoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    freycoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    freycoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    freycoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[freycoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of freycoind>")
        exit(1)
