...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Freycoin network. This means one can have an always-on freycoind instance running, and another local freycoind/freycoin-qt instance which connects to this node and receives blocks from it.
