...
function getBlockchainInfo() {
	return tryCacheThenRpcApi(miscCache, "getBlockchainInfo", 10 * ONE_SEC, async () => {
		let info = await rpcApi.getBlockchainInfo();

		info.difficulty = convertFreycoinDifficulty(info.difficulty);

		return info;
	});
}

// Freycoin difficulty is a uint64 hex string with 2^48 fixed-point precision.
// Convert exactly to a Decimal string using BigInt arithmetic (zero precision loss).
// Returns the original value unchanged if it's already a number (Bitcoin-style).
function convertFreycoinDifficulty(diff) {
	if (typeof diff === "string" && /^[0-9a-fA-F]+$/.test(diff)) {
		let raw = BigInt("0x" + diff);
		let shift = BigInt(48);
		let divisor = BigInt(1) << shift;
		let intPart = raw / divisor;
		let remainder = raw % divisor;

		// Compute fractional digits via repeated BigInt multiplication (exact)
		// 15 decimal digits covers 48 bits of fractional precision fully
		let fracDigits = 15;
		let fracScaled = remainder * (BigInt(10) ** BigInt(fracDigits)) / divisor;
		let fracStr = fracScaled.toString().padStart(fracDigits, "0");
...
function getBlockByHeight(blockHeight) {
	return tryCacheThenRpcApi(blockCache, "getBlockByHeight-" + blockHeight, FIFTEEN_MIN, async function() {
		let block = await rpcApi.getBlockByHeight(blockHeight);
		if (block && block.difficulty) {
			block.difficulty = convertFreycoinDifficulty(block.difficulty);
		}
		return block;
	});
}
...
function getBlockByHash(blockHash) {
	return tryCacheThenRpcApi(blockCache, "getBlockByHash-" + blockHash, FIFTEEN_MIN, async function() {
		let block = await rpcApi.getBlockByHash(blockHash);
		if (block && block.difficulty) {
			block.difficulty = convertFreycoinDifficulty(block.difficulty);
		}
		return block;
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			for (let i = startBlock; i <= endBlock; i++) {
				// Skip genesis block — its coinbase is OP_RETURN with no miner address
				if (i === 0) {
					markItemsDone(3);
					continue;
				}
			...
			for (let height = startBlock; height <= endBlock; height++) {
				if (height === 0) continue; // Skip genesis block
