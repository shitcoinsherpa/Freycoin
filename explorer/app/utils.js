...
function satoshisPerUnitOfLocalCurrency(localCurrency) {
	if (global.exchangeRates != null) {
		...
		let baseCurrencyUnit = coins[config.coin].baseCurrencyUnit;
		let baseCurrencyType = global.currencyTypes[baseCurrencyUnit.name.toLowerCase()] || global.currencyTypes["subfrey"] || global.currencyTypes["sat"];
		let localCurrencyType = global.currencyTypes[localCurrency];

		// FREY/USD -> subfrey/USD
		dec = dec.times(baseCurrencyType.multiplier);

		let exchangedAmt = parseInt(dec);

		return {amt:addThousandsSeparators(exchangedAmt),amtRaw:exchangedAmt, unit:`${baseCurrencyUnit.name}/${localCurrencyType.symbol}`}
	}
