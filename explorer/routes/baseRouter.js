...
const primeGapData = require("./../app/primeGapData.js");
...
// Disabled routes — these endpoints are not exposed on public-facing instances
const disabledRoutes = ["/rpc-terminal", "/rpc-browser", "/fun", "/holidays", "/utxo-set"];
...
router.get("/", asyncHandler(async (req, res, next) => {
	try {
		...
		// Discovery stats for the math-first landing page
		res.locals.discoveryStats = null;
		try {
			let globalSummary = primeGapData.getRecordSummary();
			if (globalSummary.totalGlobal > 0) {
				// Quick scan for chain discovery stats: unique gap sizes and highest merit in last 24h
				let uniqueGaps = new Set();
				let potentialRecords = 0;
				let highestMerit24h = 0;
				let highestMerit24hBlock = null;
				let now = Math.floor(Date.now() / 1000);
				let oneDayAgo = now - 86400;
				let chainHeight = getblockchaininfo.blocks;
				let batchSize = 20;

				for (let batchStart = 1; batchStart <= chainHeight; batchStart += batchSize) {
					let batchEnd = Math.min(batchStart + batchSize - 1, chainHeight);
					let batchPromises = [];

					for (let h = batchStart; h <= batchEnd; h++) {
						batchPromises.push((async (height) => {
							try {
								let blockHash = await rpcApi.getRpcDataWithParams({method:"getblockhash", parameters:[height]});
								let block = await rpcApi.getRpcDataWithParams({method:"getblock", parameters:[blockHash, 1]});
								if (block.gap && block.gap > 0) {
									uniqueGaps.add(block.gap);
									let gapMap = primeGapData.getAllGaps();
									let globalRecord = gapMap.get(block.gap);
									if (!globalRecord || (block.merit && block.merit > globalRecord.merit)) {
										potentialRecords++;
									}
									if (block.time >= oneDayAgo && block.merit > highestMerit24h) {
										highestMerit24h = block.merit;
										highestMerit24hBlock = block.height;
									}
								}
							} catch (e) { /* skip */ }
						})(h));
				...
				res.locals.discoveryStats = {
					blocksMined: chainHeight,
					uniqueGapSizes: uniqueGaps.size,
					potentialRecords: potentialRecords,
					highestMerit24h: highestMerit24h,
					highestMerit24hBlock: highestMerit24hBlock,
					globalSummary: globalSummary,
				};
			}
		} catch (e) {
			debugLog(`Could not load discovery stats: ${e.message}`);
		}
...
router.get("/mining-summary", asyncHandler(async (req, res, next) => {
	try {
		...
		// Compute actual blocks-per-day from real timestamps
		let tipHeight = getblockchaininfo.blocks;
		let lookbackHeight = Math.max(0, tipHeight - Math.min(1000, tipHeight));
		if (tipHeight > 0 && lookbackHeight < tipHeight) {
			let [tipHeader, lookbackHeader] = await Promise.all([
				coreApi.getBlockHeaderByHeight(tipHeight),
				coreApi.getBlockHeaderByHeight(lookbackHeight)
			]);
			let timeSpan = tipHeader.time - lookbackHeader.time;
			if (timeSpan > 0) {
				res.locals.actualBlocksPerDay = Math.round((tipHeight - lookbackHeight) * 86400 / timeSpan);
			}
		}
...
router.get("/block-height/:blockHeight", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Freycoin Block #${blockHeight.toLocaleString()}`;
			res.locals.metaDesc = "";
		}
...
router.get("/block/:blockHash", asyncHandler(async (req, res, next) => {
	try {
		// If the parameter looks like a block height (numeric), redirect
		if (/^\d+$/.test(req.params.blockHash)) {
			res.redirect(`${config.baseUrl}block-height/${req.params.blockHash}`);
			return;
		}
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Freycoin Block ${utils.ellipsizeMiddle(res.locals.result.getblock.hash, 16)}`;
			res.locals.metaDesc = "";
		}
		...
		res.locals.userMessageMarkdown = `Failed to load block: **${req.params.blockHash}**`;
...
router.get("/quotes", function(req, res, next) {
	...
	listNewFirst.sort(function(a, b) {
		let dateA = a.date || "0000";
		let dateB = b.date || "0000";
		let dateCompare = dateB.localeCompare(dateA);
	...
	listNewFirst.forEach(item => {
		let itemDate = item.date || "Unknown";
		let itemYear = itemDate.substring(0, 4);
	...
	listNewFirst.forEach(item => {
		let itemDate = item.date || "00-00";
		let itemMonth = itemDate.length >= 7 ? itemDate.substring(5, 7) : "00";
...
// Prime Gap Records page - scans full chain for top gaps by merit
router.get("/prime-gap-records", asyncHandler(async (req, res, next) => {
	try {
		const coinConfig = coins[config.coin];

		// Only available for coins with prime gap PoW
		if (!coinConfig.primeGapPoW) {
			res.redirect("./");
			return next();
		}

		let categoryFilter = req.query.category || "all";

		let getblockchaininfo = await coreApi.getBlockchainInfo();
		let chainHeight = getblockchaininfo.blocks;

		// Scan ALL blocks in parallel batches for accuracy
		let allRecords = [];
		let totalMerit = 0;
		let meritCount = 0;
		let uniqueGapSizes = new Set();
		let batchSize = 20;
			...
			for (let h = batchStart; h <= batchEnd; h++) {
				batchPromises.push((async (height) => {
					try {
						...
						if (block.gap && block.gap > 0) {
							let sp = (block.start_prime || "").toString().replace(/^0x/, "");
							let ep = (block.end_prime || "").toString().replace(/^0x/, "");
							let primeBits = sp.length * 4;
		...
		allRecords.sort((a, b) => b.merit - a.merit);

		// Keep top 100
		allRecords = allRecords.slice(0, 100);

		// Cross-reference with global Prime Gap List Project records (multi-category)
		let annotated = primeGapData.crossReference(allRecords);

		// Count badges by category
		let categoryCounts = { first: 0, merit: 0, maximal: 0, certified: 0 };
		for (let r of annotated) {
			for (let badge of (r.badges || [])) {
				if (badge.type === "FIRST_OCCURRENCE") categoryCounts.first++;
				if (badge.type === "MERIT_RECORD") categoryCounts.merit++;
				if (badge.type === "MAXIMAL") categoryCounts.maximal++;
				if (badge.type === "CERTIFIED_UPGRADE") categoryCounts.certified++;
			}
		}
		...
		if (categoryFilter !== "all") {
			let typeMap = {
				...
				"merit": "MERIT_RECORD",
				"maximal": "MAXIMAL",
				"certified": "CERTIFIED_UPGRADE",
			};
			let filterType = typeMap[categoryFilter];
			if (filterType) {
				filtered = annotated.filter(r => r.badges && r.badges.some(b => b.type === filterType));
			}
		}

		let potentialNewRecords = annotated.filter(r =>
			r.badges && r.badges.some(b => b.type === "FIRST_OCCURRENCE" || b.type === "MERIT_RECORD")
		).length;

		res.locals.records = filtered;
		res.locals.allRecordCount = annotated.length;
		res.locals.chainHeight = chainHeight;
		res.locals.avgMerit = meritCount > 0 ? totalMerit / meritCount : 0;
		res.locals.potentialNewRecords = potentialNewRecords;
		res.locals.globalDataLoaded = primeGapData.getAllGaps().size > 0;
		res.locals.categoryFilter = categoryFilter;
		res.locals.categoryCounts = categoryCounts;
		res.locals.uniqueGapSizes = uniqueGapSizes.size;

		res.render("prime-gap-records");

		next();

	} catch (err) {
		res.locals.pageErrors.push(utils.logError("prime-gap-records-err", err));
		res.locals.userMessage = "Error loading prime gap records: " + err;
		res.locals.records = [];
		res.locals.allRecordCount = 0;
		res.locals.chainHeight = 0;
		res.locals.avgMerit = 0;
		res.locals.categoryFilter = "all";
		res.locals.categoryCounts = { first: 0, merit: 0, maximal: 0, certified: 0 };
		res.locals.uniqueGapSizes = 0;
...
// Global Gap Records - browse the Prime Gap List Project database
router.get("/global-gap-records", asyncHandler(async (req, res, next) => {
	try {
		const coinConfig = coins[config.coin];
		...
		let minMerit = parseFloat(req.query.minMerit) || 0;
		let limit = 100;
		let offset = parseInt(req.query.offset) || 0;
		let sort = req.query.sort || "gapsize";

		let gapMap = primeGapData.getAllGaps();
		let credits = primeGapData.getCredits();

		// Convert to array and apply filters
		let allGaps = Array.from(gapMap.values());

		if (filter === "maximal") {
			allGaps = allGaps.filter(g => g.ismax === 1);
		}
		if (filter === "merit10") {
			minMerit = 10;
		} else if (filter === "merit20") {
			minMerit = 20;
		} else if (filter === "merit30") {
			minMerit = 30;
		}
		if (minMerit > 0) {
			allGaps = allGaps.filter(g => g.merit >= minMerit);
		}

		// Sort
		if (sort === "merit") {
			allGaps.sort((a, b) => b.merit - a.merit);
		} else if (sort === "year") {
			allGaps.sort((a, b) => b.year - a.year);
		} else if (sort === "digits") {
			allGaps.sort((a, b) => b.primedigits - a.primedigits);
		} else {
			allGaps.sort((a, b) => a.gapsize - b.gapsize);
		}

		// Build chain gap map for cross-referencing (gapsize → best chain merit)
		// Use cached chain data if available from a recent /prime-gap-records scan
		let chainGapMap = new Map();
		try {
			let getblockchaininfo = await coreApi.getBlockchainInfo();
			let chainHeight = getblockchaininfo.blocks;
			let batchSize = 20;
				...
				for (let h = batchStart; h <= batchEnd; h++) {
					batchPromises.push((async (height) => {
						try {
							let blockHash = await rpcApi.getRpcDataWithParams({method:"getblockhash", parameters:[height]});
							let block = await rpcApi.getRpcDataWithParams({method:"getblock", parameters:[blockHash, 1]});
							if (block.gap && block.gap > 0 && block.merit) {
								let existing = chainGapMap.get(block.gap);
								if (!existing || block.merit > existing.merit) {
									chainGapMap.set(block.gap, { gap: block.gap, merit: block.merit, height: block.height });
								}
							}
						} catch (e) { /* skip */ }
					})(h));
			...
			debugLog(`Could not load chain data for cross-reference: ${e.message}`);
		}

		// Build full chain records with start/end primes for the Freycoin Discoveries view
		let fullChainRecords = [];
		try {
			let getblockchaininfo2 = await coreApi.getBlockchainInfo();
			let chainHeight2 = getblockchaininfo2.blocks;
			let batchSize2 = 20;

			for (let batchStart = 1; batchStart <= chainHeight2; batchStart += batchSize2) {
				let batchEnd = Math.min(batchStart + batchSize2 - 1, chainHeight2);
				let batchPromises = [];
				...
				for (let r of results) {
					if (r) fullChainRecords.push(r);
				}
			}
		} catch (e) {
			debugLog(`Could not load full chain records: ${e.message}`);
		}

		// Cross-reference chain records for multi-category badges
		let annotatedChain = primeGapData.crossReference(fullChainRecords);

		// Count record-worthy chain discoveries
		let chainRecordWorthy = [];
		let chainMatchCount = chainGapMap.size;
		let chainHigherMeritCount = 0;
		let chainNewGapCount = 0;

		for (let r of annotatedChain) {
			let badges = r.badges || [];
			let isRecordWorthy = badges.some(function(b) {
				return b.type === "FIRST_OCCURRENCE" || b.type === "MERIT_RECORD" ||
				       b.type === "MAXIMAL" || b.type === "CERTIFIED_UPGRADE";
			});
			if (isRecordWorthy) {
				chainRecordWorthy.push(r);
			}
			for (let b of badges) {
				if (b.type === "FIRST_OCCURRENCE") chainNewGapCount++;
				if (b.type === "MERIT_RECORD") chainHigherMeritCount++;
			}
		}

		// Sort record-worthy by merit descending
		chainRecordWorthy.sort(function(a, b) { return b.merit - a.merit; });

		// For non-freycoin filters, apply the standard global DB filtering
		if (filter !== "freycoin") {
			// Compute stats from the (possibly filtered) global data
			let largestGap = 0;
			let highestMerit = 0;
			let newestYear = 0;
			for (let g of allGaps) {
				if (g.gapsize > largestGap) largestGap = g.gapsize;
				if (g.merit > highestMerit) highestMerit = g.merit;
				if (g.year > newestYear) newestYear = g.year;
			}

			let totalCount = allGaps.length;
			let paged = allGaps.slice(offset, offset + limit);

			res.locals.gaps = paged;
			res.locals.totalCount = totalCount;
			res.locals.largestGap = largestGap;
			res.locals.highestMerit = highestMerit;
			res.locals.newestYear = newestYear;
		} else {
			// Freycoin Discoveries view — show our record-worthy chain records
			res.locals.gaps = [];
			res.locals.totalCount = 0;
			res.locals.largestGap = 0;
			res.locals.highestMerit = 0;
			res.locals.newestYear = 0;
		}

		res.locals.credits = credits;
		res.locals.chainGapMap = chainGapMap;
		res.locals.filter = filter;
		res.locals.sort = sort;
		res.locals.offset = offset;
		res.locals.limit = limit;
		res.locals.lastUpdated = primeGapData.getLastUpdated();
		res.locals.chainMatchCount = chainMatchCount;
		res.locals.chainHigherMeritCount = chainHigherMeritCount;
		res.locals.chainNewGapCount = chainNewGapCount;
		res.locals.chainRecordWorthy = chainRecordWorthy;

		res.render("global-gap-records");

		next();

	} catch (err) {
		res.locals.pageErrors.push(utils.logError("global-gap-records-err", err));
		res.locals.userMessage = "Error loading global gap records: " + err;
		res.locals.gaps = [];
		res.locals.credits = new Map();
		res.locals.chainGapMap = new Map();
		res.locals.filter = "all";
		res.locals.sort = "gapsize";
		res.locals.totalCount = 0;
		res.locals.offset = 0;
		res.locals.limit = 100;
		res.locals.lastUpdated = null;
		res.locals.largestGap = 0;
		res.locals.highestMerit = 0;
		res.locals.newestYear = 0;
		res.locals.chainMatchCount = 0;
		res.locals.chainHigherMeritCount = 0;
		res.locals.chainNewGapCount = 0;
		res.locals.chainRecordWorthy = [];
...
// Missing Gaps page - gap sizes not yet in the global database
router.get("/missing-gaps", asyncHandler(async (req, res, next) => {
	try {
		const coinConfig = coins[config.coin];
		...
		// Find where missing gaps actually start for smart defaults
		let firstMissing = primeGapData.getFirstMissingGap();
		let defaultMin = firstMissing ? firstMissing - 100 : 100000;
		if (defaultMin < 2) defaultMin = 2;
		let defaultMax = firstMissing ? firstMissing + 10000 : 200000;

		let minSize = parseInt(req.query.min) || defaultMin;
		let maxSize = parseInt(req.query.max) || defaultMax;

		// Clamp range to reasonable bounds
		if (minSize < 2) minSize = 2;
		if (maxSize > 2000000) maxSize = 2000000;
		if (maxSize < minSize) maxSize = minSize + 1000;
		if (maxSize - minSize > 500000) maxSize = minSize + 500000;

		let gapMap = primeGapData.getAllGaps();
		let missingGaps = primeGapData.getMissingGaps(minSize, maxSize);

		// Build chain gap set for highlighting
		let chainGapSet = new Set();
		try {
			let getblockchaininfo = await coreApi.getBlockchainInfo();
			let chainHeight = getblockchaininfo.blocks;
			let batchSize = 20;
			...
			debugLog(`Could not load chain data for missing gaps: ${e.message}`);
		}

		// For each missing gap, find nearest known gaps above and below
		let gapSizes = Array.from(gapMap.keys()).sort((a, b) => a - b);
		let missingGapsWithContext = missingGaps.map(g => {
			let nearestBelow = null;
			let nearestAbove = null;

			// Binary search for nearest below
			for (let i = gapSizes.length - 1; i >= 0; i--) {
				if (gapSizes[i] < g) { nearestBelow = gapSizes[i]; break; }
			}
			// Binary search for nearest above
			for (let i = 0; i < gapSizes.length; i++) {
				if (gapSizes[i] > g) { nearestAbove = gapSizes[i]; break; }
			}

			return {
				gapSize: g,
				chainFound: chainGapSet.has(g),
				nearestBelow: nearestBelow,
				nearestAbove: nearestAbove,
			};
		});

		// Count how many even gaps exist vs are known in the range
		let totalEvenInRange = Math.floor((maxSize - minSize) / 2) + 1;
		let knownInRange = totalEvenInRange - missingGaps.length;

		res.locals.missingGaps = missingGapsWithContext;
		res.locals.minSize = minSize;
		res.locals.maxSize = maxSize;
		res.locals.totalMissing = missingGaps.length;
		res.locals.totalEvenInRange = totalEvenInRange;
		res.locals.knownInRange = knownInRange;
		res.locals.globalDataLoaded = gapMap.size > 0;
		res.locals.lastUpdated = primeGapData.getLastUpdated();
		res.locals.firstMissingGap = firstMissing;

		res.render("missing-gaps");

		next();

	} catch (err) {
		res.locals.pageErrors.push(utils.logError("missing-gaps-err", err));
		res.locals.userMessage = "Error loading missing gaps: " + err;
		res.locals.missingGaps = [];
		res.locals.minSize = 2;
		res.locals.maxSize = 2000;
		res.locals.totalMissing = 0;
		res.locals.totalEvenInRange = 0;
		res.locals.knownInRange = 0;
		res.locals.globalDataLoaded = false;
		res.locals.lastUpdated = null;
