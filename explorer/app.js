...
// View caching disabled — live data explorer, template changes take effect immediately
...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
	...
	// Prime Gap List Project data (global records cross-referencing)
	const coinConfig = coins[config.coin];
	if (coinConfig.primeGapPoW) {
		primeGapData.initialize();
		setInterval(primeGapData.refreshData, 6 * 60 * 60 * 1000);
		global.primeGapData = primeGapData;
	}
