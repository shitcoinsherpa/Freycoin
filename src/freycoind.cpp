...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/freycoin.conf are parsed in qt/bitcoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see freycoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for freycoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect freycoind signal handlers
    noui_connect();
