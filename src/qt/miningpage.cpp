...
void MiningPage::miningThreadFunc()
{
    ...
    while (!m_stopRequested.load()) {
        // Pause mining if node enters IBD (e.g. reorg or resync)
        if (ctx->chainman->IsInitialBlockDownload()) {
            QMetaObject::invokeMethod(this, [this]() {
                logMessage("Pausing mining — blockchain is syncing...");
            }, Qt::QueuedConnection);
            while (!m_stopRequested.load() && ctx->chainman->IsInitialBlockDownload()) {
                std::this_thread::sleep_for(std::chrono::seconds(5));
            }
            if (m_stopRequested.load()) break;
            QMetaObject::invokeMethod(this, [this]() {
                logMessage("Sync complete, resuming mining.");
            }, Qt::QueuedConnection);
        }
        ...
        try {
            ...
            if (processor.found) {
                ...
                // Determine block height for fork-aware PoW check
                int nBlockHeight = -1;
                {
                    LOCK(cs_main);
                    const CBlockIndex* pindexPrev = ctx->chainman->m_blockman.LookupBlockIndex(block.hashPrevBlock);
                ...
                if (CheckProofOfWork(block, nBlockHeight, ctx->chainman->GetConsensus())) {
