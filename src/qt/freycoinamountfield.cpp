...
FreycoinAmountField::FreycoinAmountField(QWidget *parent): QLineEdit(parent)
{
...
bool FreycoinAmountField::validate()
{
    bool valid{false};
...
void FreycoinAmountField::setValue(const CAmount& value) {
    setText(GUIUtil::formatAmount(value, false, GUIUtil::SeparatorStyle::NEVER));
    validate();
    Q_EMIT valueChanged();
}

CAmount FreycoinAmountField::parse(const QString &text, bool *valid_out) const
{
    CAmount val = 0;
...
bool FreycoinAmountField::event(QEvent *event)
{
