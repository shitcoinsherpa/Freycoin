...
bool fermat320(__private uint *p) {
    ...
    for (int i = 2; i < 8; i++) { for (int j = 0; j < 10; j++) window[i][j] = window[i-1][j]; monMul320(window[i], window[1], mod, invm); }
...
bool fermat352(__private uint *p) {
    ...
    for (int i = 2; i < 8; i++) { for (int j = 0; j < 11; j++) window[i][j] = window[i-1][j]; monMul352(window[i], window[1], mod, invm); }
