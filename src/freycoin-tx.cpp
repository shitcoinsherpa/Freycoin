...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " freycoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The freycoin-tx tool is used for creating and modifying bitcoin transactions.\n\n"
                "freycoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded Freycoin transaction, or with \"-create [commands]\" to create a hex-encoded bitcoin transaction.\n"
                "\n"
                "Usage: freycoin-tx [options] <hex-tx> [commands]\n"
                "or:    freycoin-tx [options] -create [commands]\n"
                "\n";
