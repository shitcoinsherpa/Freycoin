...
// Copyright (c) 2025 The Freycoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries freycoin-node, freycoin-gui.
  -M, --monolithic       Run monolithic binaries freycoind, freycoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'freycoin-qt [ARGS]' or 'freycoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'freycoind [ARGS]' or 'freycoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'freycoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'freycoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'freycoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_freycoin [ARGS]'.
  chainstate [ARGS] Run freycoin kernel chainstate util, equivalent to running 'freycoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_freycoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_freycoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "freycoin-gui" : "freycoin-qt");
            ...
            args.emplace_back("freycoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("freycoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_freycoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("freycoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_freycoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_freycoin-qt");
        } else {
...
//! Execute the specified freycoind, freycoin-qt or other command line in `args`
...
//!             be a relative path to a freycoind, freycoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/freycoin ...
